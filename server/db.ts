...
// Video analysis database helpers

export async function createVideoAnalysis(data: InsertVideoAnalysis): Promise<VideoAnalysis> {
  const db = await getDb();
  if (!db) {
  ...
  const inserted = await db.select().from(videoAnalyses).where(eq(videoAnalyses.id, insertedId)).limit(1);
  if (inserted.length === 0) {
    throw new Error("Failed to retrieve inserted video analysis");
  }
  
  return inserted[0];
}

export async function updateVideoAnalysis(id: number, data: Partial<InsertVideoAnalysis>): Promise<void> {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
...
export async function getVideoAnalysisById(id: number): Promise<VideoAnalysis | undefined> {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
